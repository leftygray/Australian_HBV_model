...
# First create some functions to help produce annual estimates
...
yearDf <- function(df, sumVar, midVar, pg, divideVar = NULL) {
  ...
  # Args:
  #   df:
  #   sumVar: String naming variable to calculate yearly summs
  #   midVar: String naming variable to extract mid year estimates from
  #   pg: project specifications
  #   divideVar: String naming variable given by sumVar / midVar. Default
  #     is NULL as it is optional
  # Returns:
  #   Dataframe with annual/yearly estimates
  # 
  # ----------------------------------------------------------------------
  
  yearFrame <- df[seq(1, nrow(df), 1 / pg$timestep), ]
  yearFrame[, sumVar] <- sumYear(df[, sumVar], pg$timestep)
  yearFrame$year <- rep(head(pg$years, -1), pg$npops)
  yearFrame[, midVar] <- df[, midVar][midyearIndex(nrow(df))]
...
annNewInfections <- yearDf(newInfections, "infections", "total_pop", pg,
                           divideVar = "incidence")
