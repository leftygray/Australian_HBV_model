...
totalPopPlot <- ggplot(data = totalPop, aes(x = time, y = population)) +
  geom_line() +
  xlab("Year") + ylab("Overall population size") +
  plotOpts
...
agePopPlot <- ggplot(data = popSizesAge, 
                     aes(x = time, y = population, group = age_group)) +
...
infectSizes <-  apply(bestResults$allPops, 3, 
                      function(x) rowSums(x[,2:pg$nstates]))
