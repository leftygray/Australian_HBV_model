...
popNewInfectionsPlot <- indicatorPlot(plotNewInfectsPops, 
                                   ylabel = "New infections",
                                   xlimits = c(startYear, endYear, 5),
                                   range = TRUE,
                                   facetPlot = "population")
...
tempPopInfections <- select(tempPopInfections, -year, -population)
...
tempPopPop <- popResults(pg, bestResults, paramResults, states = "all",
                       range = FALSE) %>% select(-year, -population)
tempPopPop <- tempTotalPop[MidyearIndex(nrow(tempPopPop),
                            pg$timestep), ]


#Calculate incidence per 100,000
tempTotalInc <- (1e5 * tempTotalInfections / tempTotalPop) %>%
...
tempPopInc <- (1e5 * tempPopInfections / tempPopPop) %>%
  mutate(year = head(pg$years, -1)) %>%
  select(year, everything()) %>%
  tbl_df()
