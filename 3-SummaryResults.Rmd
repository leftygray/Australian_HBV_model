...
# Have produced a plot of this
infectSizes <-  popSizes %>%
  group_by(year, age_group) %>%
  filter(state != "s") %>% # exclude susceptibles; Shouldn't "cleared"" and "immune" be excluded too?
...
totalInfectPlot <- ggplot(data = totalInfect, aes(x = year, y = totalinfect)) +
  geom_line(colour = "blue") +
  xlab("Year") + ylab("Population size") +
  scale_x_continuous(breaks = seq(pg$start_year, pg$end_year +1, 
                                  by = 20)) +
...
# Plot of population size in every group
plotInfectPops <- infectSizes
plotInfectPops$age_group <- factor(infectSizes$age_group)
levels(plotAgePops$age_group) <- c("Age < 4", "Age 5 to 14", 
                                   "Age 15 to 44", "Age > 45") # rename levels for plotting

infectPopPlot <- ggplot(data = plotInfectPops, 
...
prevPopPlot <- ggplot(data = plotPrevPops, 
...
# Neil's code ------------------------------------------------------------
# Do we still need these below? I cleaned it up a bit
